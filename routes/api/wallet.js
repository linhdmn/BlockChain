	...
	*/
	...
	.then(
		function() {
			...
			// the next .then in the chain
			return database.addDocument(config.walletCollection, requestBody)
		}) 	// No function is provided to handle the connection failing and so that
			// error will flow through to the next .then
	.then(
		function(docs) {
	...
	.then(
		function() {
			...
			var checkIn = {
				...
				idwallet: requestBody.idwallet
			}

			// Returning will pass the promise returned by addDoc to
			// the next .then in the chain
			return database.addDocument(config.walletCollection, checkIn)
		}) 	// No function is provided to handle the connection failing and so that
			// error will flow through to the next .then
	.then(
		function(docs) {
	...
	*/
	...
	.then(
		function() {
			...
			// the next .then in the chain
			return database.countDocuments(config.walletCollection)
		}) 	// No function is provided to handle the connection failing and so that
			// error will flow through to the next .then
	.then(
		function(count) {
	...
	*/
	...
	.then(
		function() {
			...
			// the next .then in the chain
			return database.mostRecentDocument(config.walletCollection)
		}) 	// No function is provided to handle the connection failing and so that
			// error will flow through to the next .then
	.then(
		function(doc) {
...
router.post('/loginRequest', function(req, res, next){
    ...
    .then(
        function(){
            ...
            return database.findEmail(config.walletCollection, requestBody);
        }
    )
    .then(
		function(docs) {
		...
		function(error) {
			console.log('Failed to find user email: ' + error);
			return error;
		})
	.then(
		function(resultObject) {
...
router.get('/allwallets', function(req, res, next){
    var requestBody = req.body;
    var database = new DB;
    ...
    .then(
        function(){
            return database.getAll(config.walletCollection);
        }
    )
    .then(
        function(docs){
