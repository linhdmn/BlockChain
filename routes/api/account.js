	...
	*/
	...
	.then(
		function() {
			...
			// the next .then in the chain
			return database.addDocument(config.userCollection, requestBody)
		}) 	// No function is provided to handle the connection failing and so that
			// error will flow through to the next .then
	.then(
		function(docs) {
	...
	*/
	...
	.then(
		function() {
			...
			// Returning will pass the promise returned by addDoc to
			// the next .then in the chain
			return database.addDocument(config.userCollection, checkIn)
		}) 	// No function is provided to handle the connection failing and so that
			// error will flow through to the next .then
	.then(
		function(docs) {
	...
	*/
	...
	.then(
		function() {
			...
			// the next .then in the chain
			return database.countDocuments(config.userCollection)
		}) 	// No function is provided to handle the connection failing and so that
			// error will flow through to the next .then
	.then(
		function(count) {
	...
	*/
	...
	.then(
		function() {
			...
			// the next .then in the chain
			return database.mostRecentDocument(config.userCollection)
		}) 	// No function is provided to handle the connection failing and so that
			// error will flow through to the next .then
	.then(
		function(doc) {
...
router.post('/loginRequest', function(req, res, next){
    ...
    .then(
        function(){
            ...
            return database.findEmail(config.userCollection, requestBody);
        }
    )
    .then(
		function(docs) {
...
router.get('/allusers', function(req, res, next){
    ...
    .then(
        function(){
            return database.getAll(config.userCollection);
        }
    )
    .then(
        function(docs){
